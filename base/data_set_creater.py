...
def data_set_reconciliation():
    ...
    for _, data in userdata.iterrows():
        ...
        if data['services_total']!=0:
            ...
            context_bills[0][0]['Company ServComp'] += -data['services_total']
        if data['compensations_total']!=0:
            user = data['user_hr_id']
            context_entries.append([
                user,
                data['compensations_total'],
                'Current Net balance',
                'Company ServComp',
                'Reconciliation',
                'Applied'
            ...
            context_bills[0][0]['Company ServComp'] += -data['compensations_total']
        if data['total_net_month'] - data['zp_cash']!=0:
            user = data['user_hr_id']
            context_entries.append([
                user,
                data['total_net_month'] - data['zp_cash'],
                'Current Net balance',
                'Company Net Income',
                'Reconciliation',
                'Applied'
            ...
            context_bills[0][2]['Company Net Income'] += data['total_net_month'] - data['zp_cash']
        ...
        if data['office_fees']!=0:
            ...
            context_bills[0][1]['Company Office Fees'] += -data['office_fees']
        ...
        if data['social']!=0:
            ...
            context_bills[0][3]['Company Social Fund'] += data['social']

        if data['account_plus_minus']!=0:
            user = data['user_hr_id']
            context_entries.append([
                user,
                data['account_plus_minus'],
                'Account',
                'Cash hub',
                'Reconciliation',
                'Applied'
...
def add_number_bills(config, bills, entries):
    session = create_user_session(**config)
